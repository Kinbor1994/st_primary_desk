...
def students_view(session: Session):
    ...
    if not active_year:
        st.warning("⚠️ Veuillez activer une année scolaire dans la section Configuration.")
        return
    ...
    if not classes:
        st.error("❌ Aucune classe n'est ouverte pour cette année. Veuillez créer des classes avant d'inscrire des élèves.")
        return
    ...
    tab_import, tab_list = st.tabs(["⬆️ Importer Fichier Excel", "📋 Liste et Enrôlement"])

    with tab_import:
        st.subheader("Importation des Données Élèves")
        st.caption("Téléchargez un fichier Excel (.xlsx ou .csv) contenant les colonnes obligatoires (MATRICULE, NOM, PRENOM, etc.).")
        
        uploaded_file = st.file_uploader("Choisir un fichier d'élèves", type=["xlsx", "csv"])
        ...
        if uploaded_file:
            if st.button("Lancer l'Importation et l'Inscription"):
                ...
                required_cols = ['MATRICULE', 'NOM', 'PRENOM', 'SEXE', 'DATE_NAISSANCE', 'LIEU_NAISSANCE', 'NOM_PARENT', 'CONTACT_PARENT']
                
                if not all(col in df.columns for col in required_cols):
                    st.error("Erreur: Le fichier doit contenir au moins les colonnes obligatoires.")
                    return
                ...
                with st.spinner("Traitement des données..."):
                    ...
                    for index, row in df.iterrows():
                        ...
                        student = session.exec(select(Student).where(Student.matricule == matricule)).first()
                        ...
                        try:
                            ...
                            st.warning(f"Ligne {index+2} ({matricule}) : Date de naissance invalide. Élève ignoré.")
                            continue
                        ...
                        if not student:
                            ...
                            student = Student(
                                ...
                                lieu_naissance=str(row['LIEU_NAISSANCE']).strip().upper(),
                                nom_parent=str(row['NOM_PARENT']).strip(),
                                contact_parent=str(row['CONTACT_PARENT']).strip(),
                                profession_parent=str(row.get('PROFESSION_PARENT', 'N/A')).strip(),
                                est_orphelin=str(row.get('ORPHELIN', 'FAUX')).upper() == 'VRAI',
                                est_demuni=str(row.get('DEMUNI', 'FAUX')).upper() == 'VRAI',
                            )
                            session.add(student)
                            session.commit()
                        ...
                        existing_enrollment = session.exec(
                            select(Enrollment).where(
                                ...
                                Enrollment.classroom_id.in_([c.id for c in classes]) # Vérifie dans toutes les classes de l'année
                            )
                        ).first()
                        
                        if existing_enrollment:
                            st.warning(f"Ligne {index+2} ({matricule}) : Déjà inscrit cette année. Enrollment ignoré.")
                        else:
                ...
                st.success(f"Opération terminée. **{total_imported}** nouveaux élèves créés. **{total_enrolled}** inscriptions enregistrées dans la classe **{selected_class_name}**.")
                st.balloons()
    ...
    with tab_list:
        ...
        # Jointure manuelle pour l'affichage (récupère Élève et Classe via l'Enrollment)
        ...
        if enrollments:
            ...
            # Note : Maintenant on parcourt les tuples (enr, stu, classroom)
            for enr, student, classroom in enrollments:
                
                # Le code pour la construction de display_data est simplifié
                # car nous avons déjà l'objet 'student' et 'classroom'
                display_data.append({
