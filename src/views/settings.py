...
def settings_view(session: Session):
    ...
    # On utilise des onglets pour séparer les infos de l'école et les années scolaires
    tab1, tab2 = st.tabs(["🏫 Informations École", "📅 Années Scolaires"])

    # --- ONGLET 1 : INFOS ÉCOLE ---
    with tab1:
        # On cherche si les infos existent déjà
        ...
        with st.form("school_info_form"):
            ...
            with col1:
                ...
                niveau = st.selectbox("Niveau", [e.value for e in NiveauEcole], index=0)
                statut = st.selectbox("Statut", [e.value for e in StatutEcole], index=0)
            ...
            if submitted:
                ...
                school_info.nom = nom
                school_info.niveau = niveau
                school_info.statut = statut
        ...
        with st.form("add_year_form", clear_on_submit=True):
            ...
            if submitted_year and new_year_name:
                ...
                if existing:
                    ...
                    st.rerun() # Rafraîchir la page pour voir la nouvelle année

        st.divider()

        # 2. Liste et Activation
